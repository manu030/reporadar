...
class AIAnalyzer {
  constructor() {
    ...
    this.systemPrompt = `Eres un experto analista de tendencias tecnológicas y negocios digitales. Tu trabajo es analizar repositorios de GitHub trending y generar ideas de negocio detalladas y viables.
...
8. Difficulty: Debe evaluar realísticamente la complejidad de implementación`;
  ...
  async analyzeRepository(repo) {
    console.log(`🤖 Analizando ${repo.name} con IA...`);
    
    // If in test mode, skip API call and use fallback
    if (this.testMode) {
      console.log(`🔄 Using fallback ideas for ${repo.name} (test mode)`);
      return this.generateFallbackIdeas(repo);
    }
    
    const userPrompt = `Analiza este repositorio de GitHub trending y genera 3 ideas de negocio específicas:
...
Responde solo con JSON válido siguiendo el formato especificado.`;
  ...
  generateFallbackIdeas(repo) {
    console.log(`🔄 Generando ideas de fallback para ${repo.name}`);
    
    const language = (repo.language || 'unknown').toLowerCase();
    const repoName = repo.repository || repo.name.split('/')[1] || repo.name || 'herramienta';
...
Responde con JSON:
{
  ...
  async batchAnalyzeRepositories(repos) {
    console.log(`🚀 Analizando ${repos.length} repositorios en lote...`);
    
    const results = [];
    
    for (const repo of repos) {
      try {
        const ideas = await this.analyzeRepository(repo);
        results.push({
          repo,
          ideas,
          success: true
        ...
        results.push({
          repo,
          ideas: this.generateFallbackIdeas(repo),
          success: false,
          error: error.message
        });
      }
