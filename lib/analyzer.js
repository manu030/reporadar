...
class AIAnalyzer {
  constructor() {
    // Prioritize Groq, fallback to OpenAI
    this.groqApiKey = process.env.GROQ_API_KEY;
    this.openaiApiKey = process.env.OPENAI_API_KEY;
    
    if (this.groqApiKey && !this.groqApiKey.includes('your_groq_api_key_here')) {
      console.log('🚀 Using Groq API (fast & efficient)');
...
Respond only with valid JSON following the specified format.`
    ...
    try {
      const model = this.isGroq ? 'llama-3.1-70b-versatile' : 'gpt-4o-mini';
      const completion = await this.client.chat.completions.create({
        ...
        temperature: 0.7,
        max_tokens: 2000, // Groq puede manejar más tokens
        response_format: { type: 'json_object' }
      }).catch(async (error) => {
        console.warn(`Fallback model error:`, error.message);
        const fallbackModel = this.isGroq ? 'mixtral-8x7b-32768' : 'gpt-3.5-turbo-1106';
        return this.client.chat.completions.create({
          ...
          temperature: 0.7,
          max_tokens: this.isGroq ? 2000 : 500,
          response_format: { type: 'json_object' }
        });
      });
...
Responde con JSON:
{
  ...
  "issues": ["lista de problemas si valid=false"]
    ...
    try {
      const model = this.isGroq ? 'llama-3.1-8b-instant' : 'gpt-4o-mini'; // Usar modelo más rápido para validación
      const completion = await this.client.chat.completions.create({
        model: model,
  ...
  async testConnection() {
    ...
    try {
      const model = this.isGroq ? 'llama-3.1-8b-instant' : 'gpt-4o-mini';
      const completion = await this.client.chat.completions.create({
        model: model,
      ...
      const result = JSON.parse(completion.choices[0].message.content);
      console.log(`✅ ${this.isGroq ? 'Groq' : 'OpenAI'} connection successful`);
      return result.test === 'ok';
      
    } catch (error) {
      console.error(`❌ Error testing ${this.isGroq ? 'Groq' : 'OpenAI'} connection:`, error.message);
      return false;
    }
  }
}
