...
<head>
  ...
  <style>
    ...
    .linkedin { margin-top: 10px; font-size: 12px; color: #666; }
  ...
  buildNewsletterContent(repoData, date, locale = 'es') {
    ...
    const templates = {
      es: {
        ...
        linkedinJoke: '(psicología inversa level 100)',
        starsText: 'estrellas'
      },
      en: {
        ...
        linkedinJoke: '(reverse psychology level 100)',
        starsText: 'stars'
      }
    };
    ...
    repoData.forEach((item, index) => {
      ...
      reposHtml += `
        <div style="margin-bottom: 40px; padding: 20px; border-left: 4px solid #FF9B9B; background-color: #fafafa;">
          <h3 style="color: #333; margin-bottom: 8px; font-size: 18px;">${index + 1}. ${repo.name}</h3>
          <p style="color: #888; margin-bottom: 4px; font-size: 12px;">${repo.stars.toLocaleString()} ${template.starsText} • ${repo.language}</p>
          <p style="color: #666; margin-bottom: 20px; font-style: italic;">${repo.description}</p>
      ...
      if (ideas.length === 0) {
        reposHtml += `<li style="margin-bottom: 8px; color: #888; font-style: italic;">⚠️ Ideas aún generándose...</li>`;
      } else {
...
<body>
  ...
  <div class="linkedin">
    <p>PD2: ${template.linkedinText} <a href="https://www.linkedin.com/in/manuelsierraarroyo/">${template.linkedinLink}</a> ${template.linkedinJoke}.</p>
  </div>
</body>
</html>`;
  }
