...
class Mailer {
  constructor() {
    ...
    // Initialize OpenAI/OpenRouter for AI descriptions
    this.initializeAI();
  }
...
<body>
  ...
  async sendDailyNewsletter(emails, repoData, locale = 'es') {
    ...
    // Pre-generate AI descriptions for all repos
    const repoDataWithAI = await this.addAIDescriptions(repoData, locale);
      
    const emailContent = this.buildNewsletterContent(repoDataWithAI, date, locale);
  ...
  buildNewsletterContent(repoData, date, locale = 'es') {
    ...
    const templates = {
      es: {
        ...
        viewAllIdeasText: 'Ver todas las ideas del día →',
        viewOnRepoRadarText: '📍 Ver en RepoRadar',
        viewOnGitHubText: '🔗 Ver en GitHub',
        ...
        signature: 'Un saludo,<br>Manu de RepoRadar',
      ...
      en: {
        ...
        viewAllIdeasText: 'View all today\'s ideas →',
        viewOnRepoRadarText: '📍 View on RepoRadar',
        ...
        signature: 'Best regards,<br>Manu de RepoRadar',
    ...
    // Function to create repo slug for URL
    ...
    repoData.forEach((item, index) => {
      // Handle both formats: direct repo data or wrapped in repo property
      const repo = item.repo || item;
      
      // Extraer ideas para el locale correcto
      let ideas = [];
      const itemIdeas = item.ideas;
      
      if (Array.isArray(itemIdeas)) {
        // Backward compatibility: if ideas is directly an array
        ideas = itemIdeas;
      } else if (itemIdeas && typeof itemIdeas === 'object' && itemIdeas[locale]) {
        // New structure: {es: [...], en: [...]}
        ideas = Array.isArray(itemIdeas[locale]) ? itemIdeas[locale] : [];
      } else if (itemIdeas && typeof itemIdeas === 'object' && itemIdeas.es) {
        // Fallback to Spanish if locale not found but structured data exists
        ideas = Array.isArray(itemIdeas.es) ? itemIdeas.es : [];
      }
      
      console.log(`📧 Processing repo ${repo.repo_name || repo.name} for ${locale}:`);
      console.log(`   - Ideas structure type: ${typeof itemIdeas}`);
      console.log(`   - Ideas extracted: ${ideas.length} ideas`);
      if (ideas.length > 0) {
        const firstIdea = ideas[0];
        const ideaText = typeof firstIdea === 'string' ? firstIdea : 
                        (firstIdea.oneliner || firstIdea.idea_oneliner || 'Unknown format');
        console.log(`   - First idea: ${ideaText}`);
      }
      ...
      reposHtml += `
        <div style="margin-bottom: 30px; padding: 20px; border-left: 4px solid #FF9B9B; background-color: #fafafa;">
          ...
          ${this.getRepositoryDescription(repo, locale, ideas, item.aiDescription)}
      ...
      if (ideas.length === 0) {
        ...
        ideas.forEach((idea, ideaIndex) => {
          // Handle multiple idea formats: string, {oneliner, ...}, {idea_oneliner, ...}
          let ideaText = 'Idea no disponible';
          
          if (typeof idea === 'string') {
            ideaText = idea;
          } else if (idea && typeof idea === 'object') {
            // New format: {oneliner, description, problem, ...}
            ideaText = idea.oneliner || 
                      // Legacy format: {idea_oneliner, idea_problem, ...}
                      idea.idea_oneliner ||
                      // Fallback
                      'Idea no disponible';
          }
      ...
      reposHtml += `</ul>
          ...
          <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #eee;">
            <p style="margin: 0; font-size: 13px; color: #666;">
              <a href="${template.webUrl}/#repo-${slugifyRepoName(repo.repo_name || repo.name) || index}" style="color: #FF9B9B; text-decoration: none; font-weight: bold;">${template.viewOnRepoRadarText}</a>
              &nbsp;&nbsp;•&nbsp;&nbsp;
              <a href="${repo.repo_url || repo.url}" target="_blank" style="color: #666; text-decoration: none;">${template.viewOnGitHubText}</a>
            </p>
          </div>
        </div>`;
    });
...
<body>
  ...
  <p style="margin-bottom: 20px;">${template.detailsText} <a href="${template.webUrl}/#ideas" style="color: #FF9B9B; text-decoration: none; font-weight: bold;">${template.viewAllIdeasText}</a></p>
  ...
  // Pre-generate AI descriptions for all repositories
  async addAIDescriptions(repoData, locale) {
    const repoDataWithAI = [];
    
    for (const item of repoData) {
      // Handle both formats: direct repo data or wrapped in repo property
      const repo = item.repo || item;
      
      // Extract ideas for the locale
      let ideas = [];
      const itemIdeas = item.ideas;
      ...
      const aiDescription = await this.generateAIDescription(repo, locale, ideas);
      
      // Add AI description to the item
      const enhancedItem = {
        ...item,
        aiDescription: aiDescription
      };
      
      repoDataWithAI.push(enhancedItem);
    }
    
    return repoDataWithAI;
  }

  // Generate AI-powered repository description
  async generateAIDescription(repo, locale, ideas) {
    if (!this.aiEnabled) {
      return null;
    }

    try {
      const repoName = repo.repo_name || repo.name || 'repository';
      const repoDescription = repo.repo_description || '';
      const language = repo.language || 'Unknown';
      const stars = repo.stars || 0;

      // Extract key concepts from ideas
      const ideaConcepts = ideas.map(idea => {
        if (typeof idea === 'string') return idea;
        return idea.oneliner || idea.problem || '';
      }).filter(text => text && text.length > 0);

      const prompt = locale === 'es' ? 
        `Escribe una descripción atractiva y única de 1-2 oraciones para el repositorio "${repoName}" que aparecerá en un newsletter de ideas de negocio.

CONTEXTO:
- Repositorio: ${repoName}
- Descripción original: ${repoDescription}
- Lenguaje: ${language}  
- Estrellas: ${stars}
- Ideas de negocio principales: ${ideaConcepts.join(', ')}

REQUISITOS:
- Debe ser diferente cada vez, no uses templates genéricos
- Enfócate en el potencial comercial/oportunidad de negocio  
- Entre 50-150 caracteres
- Tone profesional pero accesible
- No menciones "GitHub" ni "repositorio"

Responde solo con la descripción, sin comillas ni formato adicional.` :
        `Write an engaging and unique 1-2 sentence description for the "${repoName}" repository that will appear in a business ideas newsletter.

CONTEXT:
- Repository: ${repoName}
- Original description: ${repoDescription}
- Language: ${language}
- Stars: ${stars}
- Main business ideas: ${ideaConcepts.join(', ')}

REQUIREMENTS:
- Must be different each time, no generic templates
- Focus on commercial potential/business opportunity
- Between 50-150 characters
- Professional but accessible tone
- Don't mention "GitHub" or "repository"

Reply only with the description, no quotes or additional formatting.`;

      const models = this.isOpenRouter ? [
      ...
      for (const model of models) {
        try {
          ...
          if (description && description.length > 30 && description.length < 200) {
            console.log(`✅ AI description generated for ${repoName}: ${description.substring(0, 50)}...`);
            return `<p style="color: #666; margin-bottom: 20px; line-height: 1.5;">${description}</p>`;
          }
        } catch (modelError) {
          console.warn(`⚠️ Model ${model} failed for description generation:`, modelError.message);
          continue;
        }
      }
  ...
  getRepositoryDescription(repo, locale = 'es', ideas = [], aiDescription = null) {
    // If we have a pre-generated AI description, use it
    if (aiDescription) {
    ...
    // Create deterministic seed from repo name to ensure consistent descriptions per repo
    const repoSeed = repoName.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    ...
    if (locale === 'es') {
      const descriptions = [
        ...
        `Software ${language ? `${language}` : 'open source'} con ${stars} estrellas que está revolucionando su sector. Los desarrolladores lo están integrando en sus proyectos.`,
        `Librería ${language ? `escrita en ${language}` : 'de código abierto'} con ${stars} estrellas. Su crecimiento indica una fuerte demanda en el mercado.`,
        `Tecnología ${language ? `${language}` : 'emergente'} con ${stars} estrellas que está captando la atención de la industria. Su adopción crece exponencialmente.`,
        `Plataforma ${language ? `basada en ${language}` : 'open source'} con ${stars} estrellas. Los equipos técnicos la están adoptando para optimizar sus workflows.`,
        `Framework ${language ? `${language}` : 'de código abierto'} con ${stars} estrellas que está simplificando el desarrollo. Su popularidad refleja su utilidad práctica.`
      ];
      const selectedDesc = descriptions[repoSeed % descriptions.length];
      return `<p style="color: #666; margin-bottom: 20px; line-height: 1.5;">${selectedDesc}</p>`;
    } else {
      const descriptions = [
        ...
        `${language ? `${language}` : 'Open source'} software with ${stars} stars that's revolutionizing its sector. Developers are integrating it into their projects.`,
        `${language ? `${language}-written` : 'Open source'} library with ${stars} stars. Its growth indicates strong market demand.`,
        `${language ? `${language}` : 'Emerging'} technology with ${stars} stars capturing industry attention. Its adoption is growing exponentially.`,
        `${language ? `${language}-based` : 'Open source'} platform with ${stars} stars. Technical teams are adopting it to optimize their workflows.`,
        `${language ? `${language}` : 'Open source'} framework with ${stars} stars that's simplifying development. Its popularity reflects practical utility.`
      ];
      const selectedDesc = descriptions[repoSeed % descriptions.length];
      return `<p style="color: #666; margin-bottom: 20px; line-height: 1.5;">${selectedDesc}</p>`;
    }
