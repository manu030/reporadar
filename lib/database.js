...
class Database {
  ...
  async getLatestIdeas(locale = null) {
    try {
      ...
      // Obtener repos de esa fecha y deduplicar
      const latestReposSnapshot = await this.db.collection('processed_repos')
        .where('processed_date', '==', latestDate)
        .orderBy('stars', 'desc')
        .get();

      // LEAN deduplication: Remove duplicates by repo_name, keep highest stars
      const uniqueRepos = [];
      const seenRepos = new Set();
      
      for (const doc of latestReposSnapshot.docs) {
        const repoName = doc.data().repo_name;
        if (!seenRepos.has(repoName)) {
          seenRepos.add(repoName);
          uniqueRepos.push(doc);
          
          // Stop at 3 unique repos
          if (uniqueRepos.length >= 3) break;
        }
      }

      // LEAN: Ideas are already in repo documents
      const repos = uniqueRepos.map(doc => {
        const data = doc.data();
